...
asmlinkage long interceptor(struct pt_regs reg) {
  ...
  /* Call original system call and return its return value. */
...
asmlinkage long my_syscall(int cmd, int syscall, int pid) {
  /* Check if everything is valid first, then split into four parts
  ...
  if (cmd == REQUEST_SYSCALL_INTERCEPT) {
    ...
    /* Flag as intercepted and save original syscall */
    table[syscall].intercepted = 1;
    table[syscall].f = sys_call_table[syscall];

    /* Send syscall to interceptor */
    ...
    /* Flag as no longer intercepted and delete pid list */
    table[syscall].intercepted = 0;
    destroy_list(syscall);

    /* Restore hijacked syscall to original */
    spin_lock(&calltable_lock);
    set_addr_rw((unsigned long)sys_call_table);
    ...
    /* All this syscall's processes are monitored */
    if (pid == 0) {
      table[syscall].monitored = 2;
    }
    /* Monitor this specified pid's process */
    else {
      table[syscall].monitored = 1;
      /* Check if the process is already being monitored; if not, monitor
       * the process */
    ...
    /* Check if syscall is being intercepted */
    if (table[syscall].intercepted == 0) {
      return -EINVAL;
    }
    /* All this syscall's processes monitored, stop them */
    if (pid == 0) {
      destroy_list(syscall);
      ...
      /* If pid is monitored, remove it from the list of monitored pids */
      ...
      /* If all pids are monitored, add pid to blacklist */
...
static int init_function(void) {
  ...
  /* Initialize list of monitored pids and set initial values for each syscall */
...
{
  /* Clear list of monitored pids for each syscall */
  int i = 0;
  for (i = 0; i < NR_syscalls; i++) {
    destroy_list(i);
  }
